...
import { checkMAC, checkRBAC, checkABAC } from '../middleware/accessControl';
...
export const evaluateAccess = async (
  ...
  const macAllowed = checkMAC(reqUser.security_level, document.classification);
  if (!macAllowed) {
  ...
  // Check if user owns document or has DAC permission before evaluating policies
  const isOwnerOrHasDAC = document.owner_id === reqUser.userId || dacAllowed;
  ...
  const policyContext = {
    ...
    // Pass ownership/DAC info so policy engine can be lenient
    isOwnerOrHasDAC,
  };

  // Policy engine evaluation: If user owns document or has DAC access, 
  // department-based policies should not block (but time/location policies still apply)
  const rulesAllowed = policyEngine.evaluate(policyContext);
  if (!rulesAllowed) {
  ...
  if (hasDepartmentPolicy && document.department && !isOwnerOrHasDAC) {
    // Only enforce ABAC if there's an active ABAC policy with department rules
    // AND user doesn't own the document or have explicit DAC grant
    abacAllowed = checkABAC(
      reqUser.department || null,
      reqUser.roles[0]?.name || 'Employee',
      document.department,
      null, // No specific role required for basic ABAC
      new Date(),
    );
  }
